    ...
    <DayPicker
      ...
      components={{
        ...
        CaptionLabel: (captionLabelProps) => {
          const { children: captionChildren, ...restCaptionProps } = captionLabelProps;
          return (
            <CaptionLabel
        ...
        MonthGrid: (monthGridCompProps) => {
          const { children: monthGridChildren, className: monthGridClassName, ...restMonthGridProps } = monthGridCompProps;
          return (
            <MonthGrid
...
function Nav({
  ...
  const handlePreviousClick = React.useCallback(() => {
    if (navView === "years") {
      ...
      onPrevClick?.(new Date(displayYears.from - yearDiff, 0, 1))
      return
    }
  ...
  const handleNextClick = React.useCallback(() => {
    if (navView === "years") {
      ...
      onNextClick?.(new Date(displayYears.from + yearDiff, 0, 1))
      return
    }
    if (!nextMonth) return
    goToMonth(nextMonth)
