...
import { SimStatusIndicator } from './SimStatusIndicator';
import { AdbUnauthorizedDialog } from './AdbUnauthorizedDialog';
...
export const AdbPanel: React.FC<AdbPanelProps> = ({ theme, onCallResult }) => {
  ...
  const [unauthorizedDeviceSerial, setUnauthorizedDeviceSerial] = useState<string>('');

  // Effet pour détecter les erreurs d'autorisation
  useEffect(() => {
    if (connectionState.error) {
      const errorMsg = connectionState.error.toLowerCase();
      if (errorMsg.includes('unauthorized') || errorMsg.includes('non autorisé') || errorMsg.includes('aucun appareil android connecté et autorisé')) {
        // Essayer d'extraire le serial depuis les logs
        const logs = getLogs();
        const lastLog = logs[logs.length - 1] || '';
        const serialMatch = lastLog.match(/([a-f0-9]{8})/i);
        
        if (serialMatch) {
          setUnauthorizedDeviceSerial(serialMatch[1]);
        }
        
        setShowUnauthorizedDialog(true);
      }
    }
  }, [connectionState.error, getLogs]);
  ...
  return (
    <div className={`p-4 rounded-xl ${cardBg} ${textColor} border ${borderColor} space-y-4`}>
        ...
        {!connectionState.isConnected ? (
          <>
            ...
            {connectionState.error && (
              <Button
                onClick={() => setShowUnauthorizedDialog(true)}
                variant="secondary"
                size="sm"
                className="px-3"
              >
      ...
      {/* Indicateur de statut SIM quand connecté */}
      {connectionState.isConnected && (
        <SimStatusIndicator className="mt-4" />
      )}
      ...
      {/* Dialog de diagnostic pour les appareils non autorisés */}
      <AdbUnauthorizedDialog
        open={showUnauthorizedDialog}
        onOpenChange={setShowUnauthorizedDialog}
        deviceSerial={unauthorizedDeviceSerial}
      />
    </div>
  );
}; 